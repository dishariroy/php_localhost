...
if ( ! function_exists( 'lalita_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function lalita_get_setting( $setting ) {
		$lalita_settings = wp_parse_args(
			get_option( 'lalita_settings', array() ),
			lalita_get_defaults()
		);

		return $lalita_settings[ $setting ];
	}
}

if ( ! function_exists( 'lalita_get_layout' ) ) {
	/**
	...
	function lalita_get_layout() {
		...
		$layout = $lalita_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_lalita-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $lalita_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $lalita_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'lalita_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'lalita_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function lalita_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $lalita_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_lalita-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'lalita_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'lalita_show_excerpt' ) ) {
	/**
...
if ( ! function_exists( 'lalita_show_title' ) ) {
	/**
	 ...
	 */
	function lalita_show_title() {
		return apply_filters( 'lalita_show_title', true );
	}
}

if ( ! function_exists( 'lalita_padding_css' ) ) {
	/**
