...
if ( ! function_exists( 'lalita_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'lalita_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function lalita_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', lalita_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $lalita_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $lalita_settings[$key], ':' ) !== false ) {
					$lalita_settings[$key] = current( explode( ':', $lalita_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $lalita_settings[$key] );

				// Grab the variants using the plain name
				$variants = lalita_get_google_font_variants( $lalita_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'lalita_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'lalita_fonts_subset','' );
		...
		$default_fonts = LALITA_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('lalita-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('lalita-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'lalita_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'lalita_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function lalita_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'lalita_fonts_customize_register' ) ) {
			// Bail if Lalita Premium is active
			return;
		}

		$defaults = lalita_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Lalita_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'lalita_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'lalita_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'lalita_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Lalita_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'lalita' ),
					'settings' => array(
						'family' => 'lalita_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'lalita_settings[body_font_weight]',
						'transform' => 'lalita_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'lalita_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Lalita_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'lalita' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'lalita_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'lalita_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'lalita_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Lalita_Range_Slider_Control(
				...
				array(
					'type' => 'lalita-range-slider',
					'description' => __( 'Line height', 'lalita' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'lalita_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'lalita_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Lalita_Range_Slider_Control(
				...
				array(
					'type' => 'lalita-range-slider',
					'description' => __( 'Paragraph margin', 'lalita' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'lalita_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'lalita_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function lalita_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'lalita_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'lalita_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'lalita_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_lalita_get_all_google_fonts_ajax', 'lalita_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function lalita_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'lalita_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'lalita_customize_nonce' ] ), 'lalita_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'lalita_customize_nonce', 'lalita_customize_nonce' );
		...
		$fonts = lalita_get_all_google_fonts();
...
if ( ! function_exists( 'lalita_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: lalita_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function lalita_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, lalita_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'lalita_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'lalita_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: lalita_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function lalita_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'lalita_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function lalita_get_font_family_css( $font, $settings, $default ) {
		$lalita_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'lalita_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $lalita_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'lalita_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . lalita_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'lalita_add_to_font_customizer_list' ) ) {
	add_filter( 'lalita_typography_customize_list', 'lalita_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function lalita_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = lalita_get_all_google_fonts();
		$select_fonts = lalita_get_all_google_fonts( apply_filters( 'lalita_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $lalita_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $lalita_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $lalita_settings[ $setting ], lalita_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $lalita_settings[ $setting ] ) ) ] = array(
				'name' => $lalita_settings[ $setting ],
		...
		if ( function_exists( 'lalita_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'lalita_secondary_nav_settings', array() ),
				lalita_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], lalita_typography_default_fonts() ) ) {
