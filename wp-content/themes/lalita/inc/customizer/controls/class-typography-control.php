...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Lalita_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Lalita_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'lalita-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'lalita-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), LALITA_VERSION, true );
			wp_enqueue_style( 'lalita-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), LALITA_VERSION );

			wp_enqueue_script( 'lalita-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'lalita-typography-selectWoo' ), LALITA_VERSION, true );
			wp_enqueue_style( 'lalita-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), LALITA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'lalita_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'lalita' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'lalita' );
			$this->json[ 'google_fonts' ] = apply_filters( 'lalita_typography_customize_list', lalita_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = lalita_typography_default_fonts();
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="lalita-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="lalita-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="lalita-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="lalita-font-transform">
					<label>
